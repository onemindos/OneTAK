<template>
    <div class='space-y-4'>
        ...
        <div class='space-y-2'>
            ...
            <input v-model='form.label' class='w-full bg-gray-800 border border-gray-600 rounded px-3 py-2 text-sm text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none' placeholder='Track density — last 24h' />
        </div>

        <div class='flex gap-2'>
            <div class='flex-1 space-y-1'>
                <label class='block text-xs text-gray-400'>Intensity</label>
                <input v-model.number='form.intensity' type='number' min='0.1' max='5' step='0.1' class='w-full bg-gray-800 border border-gray-600 rounded px-2 py-1.5 text-sm text-white focus:border-blue-500 focus:outline-none' />
            </div>
            <div class='flex-1 space-y-1'>
                <label class='block text-xs text-gray-400'>Radius (px)</label>
                <input v-model.number='form.radiusPixels' type='number' min='10' max='200' step='5' class='w-full bg-gray-800 border border-gray-600 rounded px-2 py-1.5 text-sm text-white focus:border-blue-500 focus:outline-none' />
            </div>
        </div>
